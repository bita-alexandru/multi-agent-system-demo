...
object SupervisorAgent extends Agent {
  ...
  def doStart(systemInstructions: Option[String], commandProps: CommandProps)
    ...
    context.log.info(colorizeActionLog(s"${Label.SupervisorId} received a START command."))
    ...
    askLlm(prompt) match {
      case Some(response) =>
        ...
        context.log.info(colorizeAgentThoughts(Label.SupervisorId, thoughts))
        ...
        context.log.info(colorizeActionLog(
          s"${Label.SupervisorId} delegated tasks to ${workerRefs.toList.map(w => getAgentIdFromName(w)).mkString(", ")}"
        ))
      case _ =>
  ...
  def doNext(systemInstructions: Option[String], commandProps: CommandProps)
    (using context: ActorContext[Command]): Behavior[Command] = {
    context.log.info(colorizeActionLog(s"${Label.SupervisorId} received a NEXT command."))
    (commandProps.input, commandProps.from) match {
      case (secret :: Nil, Some(workerRef)) =>
        context.log.info(colorizeActionLog(s"${Label.SupervisorId} received a taskwork completion confirmation from ${getAgentIdFromName(workerRef.path.name)}."))
      ...
      case _ =>
        context.log.info(colorizeErrorLog(err(s"${getAgentIdFromName(context.self.path.name)} received unknown confirmation")))
  ...
  def doReview(systemInstructions: Option[String], commandProps: CommandProps)
    (using context: ActorContext[Command]): Behavior[Command] = {
    context.log.info(colorizeActionLog(s"${Label.SupervisorId} received a REVIEW command."))
      ...
      case secret :: Nil =>
        ...
        if (profileFile.exists() && docsDirectory.exists()) {
          ...
          if (Desktop.isDesktopSupported && Desktop.getDesktop.isSupported(Desktop.Action.BROWSE)) {
            ...
            context.log.info(colorizeErrorLog(err(s"${getAgentIdFromName(context.self.path.name)} can't open the documents in your browser")))
            context.log.debug(err("SupervisorAgent doReview Desktop.Action.Browse not supported"))
          }
        }
        val userFeedback = takeInput("Check the documents. Does everything look good?").getOrElse("yes")
        context.self ! Command.End(props = CommandProps(input = List(secret, userFeedback)))
      case _ =>
        context.log.info(colorizeErrorLog(err(s"${getAgentIdFromName(context.self.path.name)} got nothing to review")))
  ...
  def doEnd(systemInstructions: Option[String], commandProps: CommandProps)
    (using context: ActorContext[Command]): Behavior[Command] = {
    context.log.info(colorizeActionLog(s"${Label.SupervisorId} received an END command."))
    val prompt = makePrompt(systemInstructions.toList.concat(commandProps.instructions), commandProps.input)
      ...
      case (Some(response), secret :: userFeedback :: Nil) =>
        ...
        if (isUserPleased == "false") {
          if (callDeleteEmployeeDocsTool(secret, context.self.path.name).isEmpty) {
            context.log.info(colorizeErrorLog(err(s"${getAgentIdFromName(context.self.path.name)}'s call to DeleteEmployeeDocs() tool failed")))
          ...
          if (callDeleteEmployeeProfileTool(secret, context.self.path.name).isEmpty) {
            context.log.info(colorizeErrorLog(err(s"${getAgentIdFromName(context.self.path.name)}'s call to DeleteEmployeeProfile() tool failed")))
  ...
  private def callDeleteEmployeeDocsTool(secret: String, caller: String)
    ...
    context.log.info(colorizeActionLog(s"${getAgentIdFromName(caller)} is calling the DeleteEmployeeDocs('$secret') tool."))
  ...
  private def callDeleteEmployeeProfileTool(secret: String, caller: String)
    (using context: ActorContext[Command]): Option[String] = {
    context.log.info(colorizeActionLog(s"${getAgentIdFromName(caller)} is calling the DeleteEmployeeProfile('$secret') tool."))
    val request = basicRequest
